...
public:
  ...
  void run()
  {
    ...
    while ( 1 ) {
      ...
      CV_Assert(depthImage.type() == CV_16UC1);
...
#endif
        {
            // scale depth
            Mat depth_flt;
            //depthImage.convertTo(depth_flt, CV_32FC1, 1.f/10000.f);
	...
	line(colorImage, Point(0, 310), Point(640, 310), Scalar(0, 0, 200), 2, 3);
	...
	if ((GazeX >= 150 && GazeX <= 490) && (GazeY >= 60 && GazeY <= 310)) {
	  //circle(colorImage, Point(320, 240), 20, Scalar(0, 0, 250), 3, 4);
          ss << " Forward ";
	  len = write(fd, &Gforward, sizeof(Gforward));
          if(len==0) {
            continue;
          }
          if(!len) { // I/Oエラー
	...
	else if ((GazeX >= 150 && GazeX <= 490) && (GazeY > 310 && GazeY <= 480)) {
	...
	*/
	...
	else if ((GazeX >= 0 && GazeX < 150) && (GazeY >= 60 && GazeY < 310)) {
	  //circle(colorImage, Point(320, 240), 20, Scalar(250, 0, 0), 3, 4);
          ss << " ForwardLeft ";
	  len = write(fd, &GforwardLeft, sizeof(GforwardLeft));
          if(len==0) {
            continue;
          }
          if(!len) { // I/Oエラー
	...
	else if ((GazeX > 490 && GazeX <= 640) && (GazeY >= 60 && GazeY < 310)) {
	  //circle(colorImage, Point(320, 240), 20, Scalar(0, 250, 0), 3, 4);
          ss << " ForwardRight ";
	  len = write(fd, &GforwardRight, sizeof(GforwardRight));
          if(len==0) {
            continue;
          }
          if(!len) { // I/Oエラー
	...
	else if ((GazeX >= 0 && GazeX < 150) && (GazeY >= 310 && GazeY <= 480)) {
	  //circle(colorImage, Point(320, 240), 20, Scalar(50, 200, 50), 3, 4);
          ss << " BackLeft ";
	...
	else if ((GazeX > 490 && GazeX <= 640) && (GazeY >= 310 && GazeY <= 480)) {
...
/*
	  ...
	  if (target_point.x >= -0.7 && target_point.x <= 0.7 && target_point.y >= 0.7) {
            circle(colorImage, Point(320, 240), 20, Scalar(0, 0, 250), 3, 4);
	    len = write(fd, &forward, sizeof(forward));
            if(len==0) {
              continue;
...
*/
	...
	iJpeg++;
      ...
      outputGazefile << GazeX << " " << GazeY << " " << kinect_point.x * 10 << " " << kinect_point.y * 10 << endl;
