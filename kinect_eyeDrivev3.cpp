...
public:
  ...
  void run()
  {
    ...
    //outputGazefile.open("result_Odom.txt", ios::out | ios::out);
    int file_count = 0;
	...
	if (flag_goal == 1 && (target_point.x < 0.8 && target_point.x > -0.8) &&
	     (target_point.y < 0.8 && target_point.y > -0.8) && (target_point.y != 0 && target_point.y != 0)) {
	...
	if (Odometry_flag == 0) {
	  ...
	  Odometry_flag = 1;
        ...
        if ((find_if(depthValue.pointIndex+661, depthValue.pointIndex+900, [](int x){return x >= 0 && x <= 800;}) != depthValue.pointIndex+900) && (find_if(depthValue.pointIndex+1, depthValue.pointIndex+660, [](int x){return x > 0 && x >= 800;}) != depthValue.pointIndex+660) && (find_if(depthValue.pointIndex+901, depthValue.pointIndex+1560, [](int x){return x > 0 && x >= 800;}) != depthValue.pointIndex+1560)) {
	  ...
	  if (target_point.x > 0 && target_point.x > 0.8 && target_point.y < 0.8) {
            circle(colorImage, Point(320, 240), 20, Scalar(0, 250, 0), 3, 4);
            len = write(fd, &forwardRight, sizeof(forwardRight));
	    //forwardFlag = false;
            if(len==0) {
	  ...
	  if (target_point.x > -0.8 && target_point.y > 0 && target_point.y > 0.8) {
            circle(colorImage, Point(320, 240), 20, Scalar(0, 0, 250), 3, 4);
	    len = write(fd, &forward, sizeof(forward));
            if(len==0) {
              continue;
	...
	iJpeg++;
      ...
      outputGazefile << GazeX << " " << GazeY << " " << kinect_point1.x * 10 << " " << kinect_point1.y * 10 << endl;
      //outputOdomfile << kinect_point1.x * 10 << " " << kinect_point1.y * 10 << endl;
      ...
      cout << " Distance.x = " << target_point.x << " Distance.y = " << target_point.y << endl;
      cout << " myPosition.x = " << kinect_point1.x * 10 << " myPosition.y = " << kinect_point1.y * 10 << endl;
      //cout << " sita " << sita << endl;
    ...
    outputGazefile.close();
    //outputOdomfile.close();
  }
