...
/*
	  ...
	  if (target_point.x >= 0.7 && target_point.y < 0.7) {
            circle(colorImage, Point(320, 240), 20, Scalar(0, 250, 0), 3, 4);
            len = write(fd, &forwardRight, sizeof(forwardRight));
	    //forwardFlag = false;
            if(len==0) {
...
*/
	...
	iJpeg++;
      ...
      outputGazefile << GazeX << " " << GazeY << " " << kinect_point.x * 10 << " " << kinect_point.y * 10
		     << " " << target_point.x << " " << target_point.y << endl;
